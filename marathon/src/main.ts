...
async function run() {
    try {
        ...
        let marathonUrl = core.getInput('marathonUrl', <InputOptions>{required: false})
        if (marathonUrl === "") {
            console.error("Please specify marathon distribution zip url")
            return
        }

        let marathonfile = core.getInput('marathonfile', <InputOptions>{required: false})
        ...
        console.log(`Installing Marathon test runner`)
        let marathon = new Marathon(marathonUrl);
        marathon.install()
