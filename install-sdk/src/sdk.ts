...
export interface AndroidSDK {
  ...
  install(url: string, cmdlineToolsVersion: string): Promise<boolean>;
...
export abstract class BaseAndroidSdk implements AndroidSDK {
  ...
  async install(url: string, cmdlineToolsVersion: String): Promise<boolean> {
    ...
    if (fs.existsSync(`${process.env.HOME}/.android`)) {
      ...
      await execWithResult(
        `mv ${process.env.HOME}/.android ${process.env.HOME}/.android.backup`,
      );
    }
    ...
    await execWithResult(
      `mv ${ANDROID_HOME}/cmdline-tools-tmp ${ANDROID_HOME}/cmdline-tools/bootstrap-version`,
    );
    ...
    let extraPaths = `${ANDROID_HOME}/cmdline-tools/${cmdlineToolsVersion}/bin:${ANDROID_HOME}/cmdline-tools/bootstrap-version/bin:${ANDROID_HOME}/bin:${ANDROID_HOME}/platform-tools:${ANDROID_HOME}/platform-tools/bin`;

    let PATH_WITHOUT_ANDROID = PATH.split(":")
      .filter((entry) => {
    ...
    await execIgnoreFailure(
      `bash -c \\\"${this.androidHome()}/cmdline-tools/bootstrap-version/bin/sdkmanager 'cmdline-tools;${cmdlineToolsVersion}'`,
    );
  ...
  async acceptLicense(): Promise<any> {
    ...
    await writeLicenseFile(
      `${this.androidHome()}/licenses/android-sdk-license`,
      "8933bad161af4178b1185d1a37fbf41ea5269c55\n" +
        "d56f5187479451eabf01fb78af6dfcb131a6481e\n" +
        "24333f8a63b6825ea9c5514f83c2829b004d1fee",
    );
    await writeLicenseFile(
      `${this.androidHome()}/licenses/android-sdk-preview-license`,
      "84831b9409646a918e30573bab4c9c91346d8abd\n",
    );
    await writeLicenseFile(
      `${this.androidHome()}/licenses/intel-android-extra-license`,
      "d975f751698a77b662f1254ddbeed3901e976f5a\n",
    );
    await writeLicenseFile(
      `${this.androidHome()}/licenses/mips-android-sysimage-license`,
      "e9acab5b5fbb560a72cfaecce8946896ff6aab9d\n",
    );
    await writeLicenseFile(
      `${this.androidHome()}/licenses/google-gdk-license`,
      "33b6a2b64607f11b759f320ef9dff4ae5c47d97a\n",
    );
    await writeLicenseFile(
      `${this.androidHome()}/licenses/android-googletv-license`,
      "601085b94cd77f0b54ff86406957099ebe79c4d6\n",
    );
    await writeLicenseFile(
      `${this.androidHome()}/licenses/android-sdk-arm-dbt-license`,
      "859f317696f67ef3d7f30a50a5560e7834b43903",
    );
  }

  async installEmulatorPackage(
    api: string,
    tag: string,
    abi: string,
    verbose: boolean,
    ...
    await execIgnoreFailure(
      `bash -c \\\"${this.androidHome()}/cmdline-tools/bootstrap-version/bin/sdkmanager emulator 'cmdline-tools;${cmdlineToolsVersion}' platform-tools 'system-images;android-${api};${tag};${abi}'${args}"`,
    );
  }
  ...
  async installPlatform(api: string, verbose: boolean): Promise<any> {
    ...
    await execIgnoreFailure(
      `bash -c \\\"${this.androidHome()}/cmdline-tools/bootstrap-version/bin/sdkmanager 'platforms;android-${api}'${args}"`,
    );
  }
  ...
  async createEmulator(
    ...
    await execIgnoreFailure(
      `bash -c \\\"echo -n no | ${this.androidHome()}/cmdline-tools/bootstrap-version/bin/avdmanager create avd -n ${name} --package \\\"system-images;android-${api};${tag};${abi}\\\" --tag ${tag}\" ${additionalOptions}`,
    );
    return new Emulator(
      this,
      name,
  ...
  async listRunningEmulators(): Promise<Array<Emulator>> {
    let output = await execIgnoreFailure(
  ...
  async parseDevicesOutput(output: string): Promise<Array<Emulator>> {
    ...
    for (let line in lines) {
      if (line.startsWith("emulator")) {
        ...
        let nameOutput = await execIgnoreFailure(
          `${this.androidHome()}/platform-tools/adb adb -s ${serial} emu avd name`,
        );
  ...
  async startAdbServer(): Promise<any> {
    await execIgnoreFailure(
      `${this.androidHome()}/platform-tools/adb start-server`,
    );
  }
}
